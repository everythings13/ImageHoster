...
public class UserControllerTest {
  ...
  // This test checks the controller logic for user signup when user requests for a registration
  // form and checks whether the logic returns the html file 'users/registration.html'
  @Test
  public void signupWithGetRequest() throws Exception {
    this.mockMvc
        .perform(get("/users/registration"))
  ...
  // This test checks the controller logic for user signup when user fills the form and send the
  // POST request to the server with the correct password type and checks whether the logic returns
  // the html file 'users/login.html'
  @Test
  ...
  // where he can enter the username and password and checks whether the logic returns the html file
  // 'users/login.html'
  @Test
  ...
  public void signinWithWrongCredentials() throws Exception {
    ...
    this.mockMvc
        .perform(post("/users/login").session(session).flashAttr("user", userSignin))
        .andExpect(view().name("users/login"))
        .andExpect(content().string(containsString("Please Login:")));
  }
  ...
  // This test checks the controller logic for user signout where he sends the POST request to the
  // server to invalidate the session and checks whether the logic returns the html file
  // 'index.html'
  @Test
  public void logout() throws Exception {
    ...
    this.mockMvc
        .perform(post("/users/logout").session(session))
